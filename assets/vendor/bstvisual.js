...
bstNode.prototype.size = 20;
bstNode.prototype.fillStyle = '#9900cc';
bstNode.prototype.fillStyleText = '#fff';
...
var view = {
	...
	'drawNode': function(node) {
		...
		nodeWrap.append('circle')
			.attr('r', node.size)
			.style({'fill': node.fillStyle, 'stroke': node.fillStyleText, 'stroke-width': 0});

		nodeWrap.append('text')
			.attr('dx', '0')
			.attr('dy', '.35em')
			.text(node.value)
			.style({'fill': node.fillStyleText,'stroke': node.fillStyleText});
...
function input() {
  ...
  var search_algo = `SEARCH (ROOT, ITEM)   <br/><br/>

Algorithm to search an item into BST.
<br/><br/>
1) IF (ITEM = ROOT → DATA) OR (ROOT = NULL)  <BR/>
	RETURN ROOT  <BR/>
...
var insert_algo = `INSERT (TREE, ITEM)  <br/><br/>

Algorithm to insert an item into BST.  <br/><br/>

1) IF TREE = NULL <BR/>
Allocate memory for TREE <BR/>
SET TREE -> DATA = ITEM <BR/>
SET TREE -> LEFT = TREE -> RIGHT = NULL <BR/>
ELSE <BR/>
IF ITEM < TREE -> DATA<BR/>
Insert(TREE -> LEFT, ITEM) <BR/>
ELSE <BR/>
Insert(TREE -> RIGHT, ITEM) <BR/>
[END OF IF] <BR/>
[END OF IF] <BR/><BR/>
2) END <BR/>`;

var delte_algo =`DELETE (TREE, ITEM)  <br/><br/>

Algorithm to delete an item into BST.  <br/><br/>
1) IF TREE = NULL <BR/>
	Write "item not found in the tree" ELSE IF ITEM < TREE -> DATA <BR/>
	Delete(TREE->LEFT, ITEM) <BR/>
ELSE IF ITEM > TREE -> DATA <BR/>
	Delete(TREE -> RIGHT, ITEM) <BR/>
ELSE IF TREE -> LEFT AND TREE -> RIGHT <BR/>
	SET TEMP = findLargestNode(TREE -> LEFT) <BR/>
	SET TREE -> DATA = TEMP -> DATA <BR/>
	Delete(TREE -> LEFT, TEMP -> DATA) <BR/>
ELSE <BR/>
	SET TEMP = TREE <BR/>
	IF TREE -> LEFT = NULL AND TREE -> RIGHT = NULL <BR/>
		SET TREE = NULL <BR/>
	ELSE IF TREE -> LEFT != NULL <BR/>
	SET TREE = TREE -> LEFT <BR/>
	ELSE <BR/>
		SET TREE = TREE -> RIGHT <BR/>
	[END OF IF] <BR/>
	FREE TEMP <BR/><BR/>
[END OF IF] <BR/>
...
function insert(){
	document.getElementById("current_algo").innerHTML = insert_algo;
}
function delet(){
	document.getElementById("current_algo").innerHTML = delte_algo;

}
function search(){
	document.getElementById("current_algo").innerHTML = search_algo;
}